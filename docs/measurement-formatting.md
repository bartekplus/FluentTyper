...
Markers are an exact, case-sensitive allowlist: common ISO 4217 codes (codes that are also words or acronyms, such as `ALL`, `TRY`, `CAD`, are excluded) plus the suffix symbols `€`, `zł`, and `kr`. It never reorders (`€100`, `$100` stay), converts (`$` ↔ `USD`), or regroups (`1,000` ↔ `1.000`). Grouped amounts such as `1.000EUR`, ranges, lowercase or unknown markers, and existing separators are left unchanged. Text inside Markdown code (backtick spans and fences), detected code editor fields, and anything typed with code mode on is left alone, which applies to measurement units as well; the rule itself does not guess at code. Tests: `tests/grammar/CurrencySpacing.test.ts`.
...
The locale allowlist is exact: `en_US`, `fr_FR`, `hr_HR`, `es_ES`, `el_GR`, `sv_SE`, `de_DE`, `pl_PL`, `pt_BR`, and `ar_SA`. Hyphenated spellings of the same tags are accepted. Bare languages, `en_GB`, `pt_PT`, and all unknown locales fail closed. Every policy inserts U+00A0 NO-BREAK SPACE between the number and unit; decimal marks remain locale-specific (`.` for `en_US` and `ar_SA`, which follows CLDR for Arabic with Latin digits; `,` elsewhere). `ar_SA` also accepts CLDR `arab` numbers: Arabic-Indic digits U+0660–U+0669 with the Arabic decimal separator `٫` (U+066B), for example `١٫٥kg` → `١٫٥ kg`. The number is copied verbatim, so the output keeps whichever digit system was typed. One number must use one system with that system's own decimal mark; mixed forms such as `1٫5`, `١.٥`, or `١5` fail closed. The Arabic group separator `٬` (U+066C) is not parsed, exactly as Latin grouping is not, so `١٬٥٠٠kg` is left unchanged. Persian digits (U+06F0–U+06F9) are not accepted, because several are visually close to Arabic-Indic digits yet differ, and no other locale accepts Arabic-Indic digits.
